...
public class Perpus {
    ...
    public Perpus(String kat, String pen, String jdl, String bhs, String pnrbtn, String eds, int hal, String sinop){
        kategori = kat;
        penulis = pen;
        judul = jdl;
        bahasa = bhs;
    ...
    public void setPenjualan(int terjual){
        penjualan = terjual;
    }
    ...
    public double cekTingkatKesamaan(Perpus objekLain) {
        
    
        int kesamaanAtribut = 0;
        int total = 0;

        if(Arrays.equals(this.penulisbanyak, objekLain.penulisbanyak)){
            kesamaanAtribut++;
        }
        total++;

        if (this.kategori.equals(objekLain.kategori)) {
            kesamaanAtribut++;
        }
        total++;

        if (this.penulis.equals(objekLain.penulis)) {
            kesamaanAtribut++;
        }
        total++;

        if (this.judul.equals(objekLain.judul)) {
            kesamaanAtribut++;
        }
        total++;

        if (this.bahasa.equals(objekLain.bahasa)) {
            kesamaanAtribut++;
        }
        total++;

        if (this.penerbitan.equals(objekLain.penerbitan)) {
            kesamaanAtribut++;
        }
        total++;

        if (this.edisi.equals(objekLain.edisi)) {
            kesamaanAtribut++;
        }
        total++;

        if (this.halaman == objekLain.halaman) {
            kesamaanAtribut++;
        }
        total++;

        if (this.sinopsis.equals(objekLain.sinopsis)) {
            kesamaanAtribut++;
        }
        total++;

        double kesamaan = 0;
        kesamaan = ((double)kesamaanAtribut / total) * 100;
    
        return kesamaan;
    }
    public void bacaFile(String pathFile){
        try{
            File FileBaru = new File(pathFile);
            Scanner io = new Scanner(FileBaru);
            String isi = io.nextLine();
            String[] isi1 = isi.split("; ");
            this.judul = isi1[0];
            this.penulis = isi1[1];
            io.close();
    ...
    public int hitungRoyalti(int harga){
        this.harga = harga;
        this.royalti = (10*harga*penjualan)/100;
        return royalti;
    }
    public int hitungRoyalti(int harga1, int persen){
        this.harga = harga1;
        this.persen = persen;
        this.royalti = (persen*harga*penjualan)/100;
        return royalti;
    }
    ...
    public void displayMessage() {
        ...
        System.out.println("Jumlah penjualan buku : " + penjualan);
        System.out.println("Jumlah Royalti : " + royalti);
    }
}
